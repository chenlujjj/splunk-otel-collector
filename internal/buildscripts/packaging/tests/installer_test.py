...
def verify_dotnet_config(container, path, exists=True):
    if not container_file_exists(container, DOTNET_AGENT_PATH) and path == DOTNET_ZEROCONFIG_PATH:
        assert not container_file_exists(container, path)

    exists = exists if container_file_exists(container, DOTNET_AGENT_PATH) else False
...
def test_installer_with_instrumentation_default(distro, arch, method):
    ...
    with run_distro_container(distro, dockerfile=dockerfile, arch=arch, buildargs=buildargs) as container:
        ...
        if method == "preload":
            ...
            if container_file_exists(container, DOTNET_AGENT_PATH):
...
def test_installer_with_instrumentation_custom(distro, arch, method, sdk):
    ...
    with run_distro_container(distro, dockerfile=dockerfile, arch=arch, buildargs=buildargs) as container:
        ...
        if sdk == "dotnet" and arch != "amd64":
            ...
            pytest.xfail("installer script successfully failed for .NET on arm64/aarch64")
        ...
        if method == "preload":
            ...
            if sdk != "dotnet" or container_file_exists(container, DOTNET_AGENT_PATH):
                verify_config_file(container, config_path, "OTEL_RESOURCE_ATTRIBUTES", config_attributes)
